...
!!! warning
	We have determined that the to-be added Interserve/Express/RPC is no-longer being designed.\
	This is due to the promised network_test branch and issues with these solutions.
...
- `#!ts flux:isconnected(invoker: Player): boolean`\
...
- `#!ts flux.pvsing(encoder: flux.encoder, compressed: string, ...: any)`\
Invoked before `:pvs(...)`.

- `#!ts flux.pasing(encoder: flux.encoder, compressed: string, ...: any)`\
Invoked before `:pas(...)`.
...
- `#!ts flux.connected(invoker: Player)`\
Invoked when a player transitions into connected state.

- `#!ts flux.disconnected(invoker: Player)`\
...
- `#!ts flux.encoder:omit(...: Player): self`\
...
- `#!ts flux.variables:onfilter(name: string, callback: function(): Player | Player[])`\
...
- `#!ts flux.variables:set(entity: Entity | Player, name: string, value: any, force?: boolean)`\
Server: updates and replicates value (staged by default, immediate when `force`).\
Client: no-op.

- `#!ts flux.variables:set(name: string, value: any, force?: boolean)`\
Server global overload.\
Client: no-op.

- `#!ts flux.variables:sync(clients?: Player | Player[])`\
Server: sends full registry snapshot (streamed).\
Client: no-op.

- `#!ts flux.variables:erase(entity: Entity | Player)`\
...
- `#!ts schema:onwrite(callback: function(buffer: flux.encoder, entity: Entity | Player | false, value: any))`\
Overrides write behavior.\
Expected return: ignored.

- `#!ts schema:onread(callback: function(buffer: flux.decoder, entity: Entity | Player | false): any)`\
Overrides read behavior.\
Expected return: decoded value.

- `#!ts schema:ondebounce(callback: function(entity: Entity | Player | false, old: any, new: any): boolean)`\
Overrides debounce behavior (`true` blocks replication/write).\
Expected return: `boolean`.

- `#!ts schema:oncondition(callback: function(entity: Entity | Player | false, value: any): boolean?)`\
Optional server-side replication gate (`false` blocks replication).\
Expected return: `false` to block, anything else to allow.

- `#!ts schema:onfilter(callback: function(entity: Entity | Player | false): Player | Player[])`\
